...
const diaryController = {
  ...
  insertDiary: async (req, res, next) => {
    ...
    try {
      ...
      let diaryInsert = await diaryModel.insertDiary(diaryData)
      if(diaryInsert == diaryData){
  ...
  updateDiary : async (req, res, next) => {
    ...
    try {
      ...
      let diaryUpdate = await diaryModel.updateDiary(diaryData, diaryId)
      if(diaryUpdate == diaryData){
        return res.status(201).json({
          status: "success",
          error: false,
